...
import { store } from '../store.js';
    ...
    export default {
        ...
        methods: {
            getMovies(search){
                axios.get(`${this.apiUrl}movie?${this.accessToken}&query=${search}`)                .then((response)=>{
...
<template>
    <nav class="navbar bg-light">
        <div class="container-fluid">
            ...
            <button class="btn btn-outline-success" type="send" @click="getMovies(searchText), searchText=''" >Cerca</button>        </div>
    </nav>
</template>
